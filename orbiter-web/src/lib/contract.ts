...
import { StacksClient, StacksAccount, TxnBuilderTypes, BCS, HexString } from 'stacks';
import {
  ContractConfig,
  CreateDomainPayload,
...
export class OrbiterContract {
  ...
  constructor(config: ContractConfig) {
    this.config = config;
    this.client = new StacksClient(config.node_url);
  }
  ...
  async getAccountBalance(address: string): Promise<number> {
    try {
      ...
      const coinResource = resources.find(
        (r) => r.type === '0x1::coin::CoinStore<0x1::stacks_coin::StacksCoin>'
      );
