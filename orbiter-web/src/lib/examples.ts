...
export async function exampleCreateDomain() {
  ...
  const account = new StacksAccount();
  ...
  const domainPayload = {
    ...
    valuation: {
      score: 850,
      market_value: utils.parseAPT('500000'), // 500K USDCx
      seo_authority: 800,
...
export async function exampleCreateListing() {
  ...
  const listingPayload = {
    ...
    price_per_share: utils.parseAPT('100'), // 100 USDCx per share
...
export async function exampleGetMarketplaceStats() {
  ...
  try {
    ...
    console.log(`Total Volume: ${utils.formatAPT(stats.total_volume)} USDCx`);
...
export function ExampleReactComponent() {
  ...
  function DomainCreator() {
    ...
    const handleCreateDomain = async () => {
      const account = new StacksAccount(); // Get from wallet
    ...
    return (
      <div>
        ...
        {stats && (
          <div>
            <p>Total Volume: {stats.total_volume} USDCx</p>
...
export async function exampleGasEstimation() {
  ...
  console.log(`Estimated cost: ${utils.formatAPT(gasEstimate * 100)} USDCx`);
}
...
export async function exampleConfigManagement() {
  ...
  const newConfig = {
    ...
    node_url: 'https://fullnode.mainnet.stackslabs.com/v1'
  };
