...
const StageIndicator = ({ stage, currentStage, title, number }: { stage: LaunchStage, currentStage: LaunchStage, title: string, number: number }) => {
  ...
  if (isComplete) {
    ...
    statusIcon = <div className="h-6 w-6 rounded-full border-2 border-border" />;
  }

  return (
    <div className={`flex items-center gap-4 p-3 rounded-lg transition-all duration-300 ${isActive ? 'bg-black/5' : ''}`}>
      <div className="flex-shrink-0">{statusIcon}</div>
      <div>
        <p className={`font-ibm-plex-mono text-xs ${isActive ? 'text-solar-yellow-text' : 'text-muted-foreground'}`}>STAGE {number}</p>
        <h3 className={`font-space-grotesk font-bold transition-colors ${isComplete ? 'text-muted-foreground' : isActive ? 'text-foreground' : 'text-gray-600'}`}>{title}</h3>
      </div>
      ...
      case "preflight":
        return (
          <div className="glass-panel p-8 rounded-lg">
            ...
            <p className="font-ibm-plex-sans text-muted-foreground mb-6">Enter your domain name to begin the tokenization process.</p>

            <div className="space-y-4 mb-6">
              <label className="font-ibm-plex-mono text-sm solar-yellow-text" htmlFor="domainName">[ DOMAIN NAME ]</label>
              <input id="domainName" placeholder="your-domain.com" value={domainName} onChange={(e) => setDomainName(e.target.value)} className="font-mono bg-card border-border text-foreground w-full rounded-md px-3 py-2" />
            </div>

            <div className="border border-border rounded-lg p-4 mb-6">
              <h4 className="font-ibm-plex-mono text-sm text-muted-foreground mb-3">[ PRE-FLIGHT CHECKLIST ]</h4>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2">{domainName ? <CheckCircle className="h-4 w-4 text-green-400" /> : <Clock className="h-4 w-4 text-muted-foreground" />}<span className="font-mono">Domain name provided</span></div>
                <div className="flex items-center gap-2"><Globe className="h-4 w-4 text-muted-foreground" /><span className="font-mono">DNS access required for verification</span></div>
              </div>
            </div>

            <button onClick={generateTxtRecord} disabled={!isWalletConnected || !domainName.trim()} className="cta-button w-full bg-gradient-to-r from-[#FE6440] to-[#FFC700] text-black font-bold font-space-grotesk px-8 py-3 rounded-lg text-md disabled:opacity-50 disabled:cursor-not-allowed">[ BEGIN VERIFICATION ]</button>
          </div>
        );
      case "authorization":
        return (
          <div className="glass-panel p-8 rounded-lg">
            ...
            <p className="font-ibm-plex-sans text-muted-foreground mb-6">Add the following TXT record to your domain's DNS settings to prove ownership.</p>

            <div className="border border-border rounded-lg p-4 mb-6 font-ibm-plex-mono text-sm">
              <h4 className="text-muted-foreground mb-3">[ DNS CONFIGURATION ]</h4>
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between"><span className="text-muted-foreground w-20">TYPE:</span><code className="bg-card px-2 py-1 rounded">TXT</code></div>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between"><span className="text-muted-foreground w-20">NAME:</span><code className="bg-card px-2 py-1 rounded">@</code></div>
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                  <span className="text-muted-foreground w-20 mt-1">VALUE:</span>
                  <div className="flex items-center gap-2 flex-1">
                    <code className="bg-card px-2 py-1 rounded break-all w-full">{txtRecord}</code>
            ...
            <div className="bg-yellow-900/20 border border-yellow-700/50 p-4 rounded-lg flex items-start gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">DNS changes can take time to propagate. You can proceed to verify once the record is live.</p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="cta-button w-full flex-1 bg-gradient-to-r from-[#FE6440] to-[#FFC700] text-black font-bold font-space-grotesk px-8 py-3 rounded-lg text-md disabled:opacity-50 disabled:cursor-not-allowed"
              >
      ...
      case "valuation":
        return (
          <div className="glass-panel p-8 rounded-lg">
            ...
            <p className="font-ibm-plex-sans text-muted-foreground mb-6">We've calculated the objective market value of your digital real estate.</p>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6 text-center">
              ...
              <div className="glass-panel-inset p-4 rounded-lg">
                ...
                <p className="font-space-grotesk text-4xl font-bold text-foreground">${valuation.marketValue.toLocaleString()}</p>
              </div>
            </div>

            <div className="border border-border rounded-lg p-4 mb-6">
              <h4 className="font-ibm-plex-mono text-sm text-muted-foreground mb-4">[ VALUATION CONSTELLATION ]</h4>
              <div className="space-y-3">
                <div className="flex items-center justify-between font-ibm-plex-mono text-sm">
                  ...
                  <span className="font-bold text-foreground">${valuation.seoAuthority.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between font-ibm-plex-mono text-sm">
                  ...
                  <span className="font-bold text-foreground">${valuation.trafficEstimate.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between font-ibm-plex-mono text-sm">
                  ...
                  <span className="font-bold text-foreground">${valuation.brandability.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between font-ibm-plex-mono text-sm">
                  <div className="flex items-center gap-2 text-gray-300"><Globe className="h-4 w-4 text-solar-yellow-text/70" />TLD Rarity</div>
                  <span className="font-bold text-foreground">${valuation.tldRarity.toLocaleString()}</span>
                </div>
              </div>
            </div>

            <button onClick={handleProceedToTokenization} className="cta-button w-full bg-gradient-to-r from-[#FE6440] to-[#FFC700] text-black font-bold font-space-grotesk px-8 py-3 rounded-lg text-md">[ INITIATE TOKENIZATION ]</button>
          </div>
        );
      case "tokenization":
        return (
          <div className="glass-panel p-8 rounded-lg">
            ...
            <p className="font-ibm-plex-sans text-muted-foreground mb-6">Confirm the details for your new Stacks Object. This will create a unique on-chain representation of your domain with its ownership divided into liquid shares.</p>
      ...
      case "complete":
        return (
          <div className="glass-panel p-8 rounded-lg text-center">
            ...
            <p className="font-ibm-plex-sans text-lg text-foreground mb-2">
              <code className="bg-card px-2 py-1 rounded-md border border-border">{domainName}</code> has achieved stable orbit.
            </p>
            <p className="font-ibm-plex-sans text-sm text-muted-foreground mb-6">
            ...
            {transactionHash && (
              <div className="border border-green-500/30 bg-green-500/10 rounded-lg p-6 mb-6 text-left">
                ...
                <div className="space-y-3">
                  <div>
                    <span className="text-muted-foreground text-sm block mb-1">Transaction Hash:</span>
                    <code className="text-xs text-foreground bg-secondary/50 px-3 py-2 rounded block break-all font-mono">
                      {transactionHash}
                    </code>
                  </div>
                  <a
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button onClick={() => { setCurrentStage("preflight"); setDomainName(""); setTxtRecord(""); setVerificationComplete(false); }} className="w-full flex-1 bg-transparent border border-border hover:bg-[#3D2D1D] transition-colors text-white font-bold font-space-grotesk px-8 py-3 rounded-lg text-md">[ LAUNCH ANOTHER ]</button>
  ...
  return (
    <div className="antialiased min-h-screen flex flex-col justify-center items-center p-4 sm:p-6 md:p-8">
      ...
      <div className="text-center mb-12">
        <h1 className="font-space-grotesk text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter text-foreground flex items-center justify-center gap-3">
          <Rocket className="h-9 w-9 text-solar-flare-text" />
          Launch Sequence
        </h1>
        <p className="font-ibm-plex-sans text-lg md:text-xl mt-2 text-muted-foreground max-w-3xl mx-auto">
