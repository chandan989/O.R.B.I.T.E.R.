...
export class APTPriceService {
  ...
  async formatWithUSD(aptAmount: number): Promise<string> {
    ...
    if (aptAmount >= 1000000) {
      return `${(aptAmount / 1000000).toFixed(1)}M USDCx (≈$${(usdValue / 1000000).toFixed(1)}M @ $${price.toFixed(2)}/USDCx)`;
    } else if (aptAmount >= 1000) {
      return `${(aptAmount / 1000).toFixed(1)}K USDCx (≈$${(usdValue / 1000).toFixed(1)}K @ $${price.toFixed(2)}/USDCx)`;
    } else {
      return `${aptAmount.toFixed(2)} USDCx (≈$${usdValue.toFixed(2)} @ $${price.toFixed(2)}/USDCx)`;
    }
  }
}
