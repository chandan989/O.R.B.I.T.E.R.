...
import { StacksAccount } from 'stacks';
import { 
  OrbiterSDK, 
  ContractConfig, 
  NetworkType,
...
interface UseOrbiterActions {
  ...
  // Domain operations
  createDomain: (account: StacksAccount, payload: CreateDomainPayload) => Promise<TransactionResult>;
  transferDomain: (account: StacksAccount, domainObject: string, newOwner: string) => Promise<TransactionResult>;
  ...
  initializeFractional: (account: StacksAccount, domainObject: string, totalSupply: number, ticker: string) => Promise<TransactionResult>;
  transferShares: (account: StacksAccount, payload: TransferSharesPayload) => Promise<TransactionResult>;
  getShareBalance: (domainObject: string, owner: string) => Promise<number>;
  getTotalSupply: (domainObject: string) => Promise<number>;
  
  // Marketplace operations
  createListing: (account: StacksAccount, payload: CreateListingPayload) => Promise<TransactionResult>;
  buyShares: (account: StacksAccount, payload: BuySharesPayload) => Promise<TransactionResult>;
  cancelListing: (account: StacksAccount, listingObject: string) => Promise<TransactionResult>;
